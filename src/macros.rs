...
///
/// This mode of `try_transmute` performs a transmutation between types of the same size. The types
/// are inferred from the calling context, so they cannot be explicitly specified.
///
...
///
/// In prefix mode, this macro transmutes a value from `SrcType` to `DstType`, provided the
/// destination type’s size is less than or equal to the source type’s size. The types `SrcType`
/// and `DstType` must be specified explicitly.
///
/// This is useful for situations where a portion of the source value can be safely interpreted as
/// a smaller destination type.
///
/// # Safety
///
...
//         let e = $e;
//         if false {
//             // Check that the sizes of the source and destination types are equal
//             Ok(unsafe {
//                 #[allow(clippy::missing_transmute_annotations)]
//                 $crate::util::macro_util::core_reexport::mem::transmute(e)
//             })
...
macro_rules! try_transmute {
    ...
    (@prefix $src_type:ty, $dest_type:ty, $e:expr) => {{
        ...
        if false {
            #[allow(clippy::missing_transmute_annotations)]
            // Compile-time check: Source size >= destination size
